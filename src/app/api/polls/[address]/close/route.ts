...
export async function POST(
  ...
  try {
    ...
    // Create close poll instruction
    const closePollIx = await (program as any).methods
      .closePoll()
      .accounts({
    ...
    console.log(`Close transaction prepared for poll: ${address}`)

    return NextResponse.json({
      success: true,
      transaction: Array.from(serializedTx),
      message: 'Transaction prepared for closing poll',
      blockhash,
      lastValidBlockHeight
    })

  } catch (error) {
    console.error('Error preparing close transaction:', error)
    return NextResponse.json({ 
      error: 'Failed to prepare close transaction',
